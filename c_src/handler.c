...
static int on_req_nonblock(h2o_handler_t *_handler, h2o_req_t *req);
static int on_req_websocket(h2o_handler_t *_handler, h2o_req_t *req);
...
h2o_nif_hdl_child_t *
h2o_nif_handler_register(ErlNifEnv *env, h2o_pathconf_t *pathconf, ERL_NIF_TERM tag, h2o_nif_hdl_type_t type)
{
    ...
    // handler->refs = NULL;
    // handler->num_refs = 0;
    (void)enif_self(env, &handler->caller);
    handler->acc.mutex = enif_mutex_create("h2o_acc_mutex");
    (void)h2o_linklist_init_anchor(&handler->acc.pending);
    handler->req.mutex = enif_mutex_create("h2o_req_mutex");
    (void)h2o_linklist_init_anchor(&handler->req.pending);
    // handler->oph = NULL;
    // handler->opt = NULL;
    // handler->multi_first = handler->multi_last = NULL;
    // handler->mtd = NULL;
    ...
    switch (handler->type) {
    case H2O_NIF_HDL_TYPE_NONBLOCK:
        child->super.on_req = on_req_nonblock;
        break;
    case H2O_NIF_HDL_TYPE_WEBSOCKET:
        child->super.on_req = on_req_websocket;
        break;
    }
    ...
    ERL_NIF_TERM msg = enif_make_tuple3(env, enif_make_atom(env, "h2o_handler"), enif_make_copy(env, handler->tag),
                                        enif_make_resource(env, (void *)handler));
    if (!enif_send(env, &handler->caller, NULL, msg)) {
...
static volatile sig_atomic_t h2o_nif_hdl_async_id = 0;
#define NEW_ASYNC_ID() (__sync_fetch_and_add(&h2o_nif_hdl_async_id, 1))
...
static int
on_req_nonblock(h2o_handler_t *_handler, h2o_req_t *req)
{
    // h2o_drv_handler_t *handler = (h2o_drv_handler_t *)_handler;
    TRACE_F("on_req_nonblock:%s:%d\n", __FILE__, __LINE__);
    h2o_nif_hdl_child_t *child = (h2o_nif_hdl_child_t *)_handler;
    h2o_nif_handler_t *handler = child->parent;
