...
h2o_nif_logger_read_start_1(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    int expected = H2O_NIF_LOGGER_STATE_PASSIVE;
    int desired = H2O_NIF_LOGGER_STATE_NOTIFY;
    do {
        if (atomic_load_explicit(&logger->state, memory_order_relaxed) == H2O_NIF_LOGGER_STATE_ACTIVE || atomic_load_explicit(&logger->num_events, memory_order_relaxed) > 0) {
            ERL_NIF_TERM msg;
        ...
        if (atomic_load_explicit(&logger->state, memory_order_relaxed) == H2O_NIF_LOGGER_STATE_NOTIFY) {
            return ATOM_ok;
        }
...
h2o_nif_logger_read_1(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    (void)atomic_store_explicit(&logger->state, H2O_NIF_LOGGER_STATE_PASSIVE, memory_order_relaxed);
